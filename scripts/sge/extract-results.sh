...
if [ "$1" = "" ]; then
    ...
    echo "Usage: $0 sge-job-name output-directory"
    echo "  e.g. $0 prostate-sge prostate-example-results"
    exit 1
fi
if [ "$2" = "" ]; then
    echo "Missing arguments!"
...
#ANT_PROJECT_NAME=`basename ${1%.xml}`
...
if [ -d $OUTPUT_BASE ]; then
    echo "Warning: $OUTPUT_BASE already exists, file copying may fail"
fi
DATA_DIR=${OUTPUT_BASE}/data
CONFIG_DIR=${OUTPUT_BASE}/config
OUTPUT_DIR=${DATA_DIR}/${ANT_PROJECT_NAME}-results

/bin/mkdir -p ${OUTPUT_DIR} ${CONFIG_DIR} ${OUTPUT_DIR}

# Copy the files used to exectute the runs
/bin/cp ${EXECUTION_DIR}/bdval.jar ${OUTPUT_BASE}
/bin/cp -r ${EXECUTION_DIR}/buildsupport ${OUTPUT_BASE}
  ...
  if [ $? -eq 0 ]; then
    ...
    do
      ...
      do
        ...
        find ${ENDPOINT_DIR} -depth 1 -exec /bin/mv \{\} ${OUTPUT_DIR}/${RESULT}/${ENDPOINT} \;
    ...
    /usr/bin/unzip -p $ZIPFILE ${NAME}/model-conditions.txt \
      ...
      >> ${OUTPUT_DIR}/model-conditions.txt
  fi
done
